<?php 
  ...
  if(isset($_POST["Enviar"]) && $_POST["Enviar"] == "Alterar"){
      $msg = $objUsuario->alteraUsuario($_POST["nome"],$_POST['sobrenome'],$_POST['setor'],$_POST['funcao'],$_POST['email'],$_POST['senha'],$_GET['usuario']);
    }
  if(isset($_POST["Excluir"]) && $_POST["Excluir"] == "Excluir"){
      $msg = $objUsuario->excluirUsuario();
    }
  $usuario = $objUsuario->getOneUsuario();
          ...
          <form action="" method="POST">
			...
			<div class="form-row">
				<div class="form-group col-md-4">
				  ...
				  <input type="text" class="form-control" id="nome" name="nome" value="<?= $usuario[0]['nome']?>" placeholder="Nome">
				</div>
				<div class="form-group col-md-4">
				  <label for="Sobrenome">Sobrenome</label>
				  <input type="text" class="form-control" id="sobrenome" name="sobrenome" value="<?= $usuario[0]['sobrenome']?>" placeholder="Sobrenome">
				</div>
				<div class="form-group col-md-4">
				 ...
				 <select id="setor" name="setor" class="form-control">
					...
					<?php foreach($setores as  $row){
					  if(!empty($row['id'])){
					  ?>
                    <option <?= ($usuario[0]['setor']==$row['id']?'selected':'');?> value="<?= $row['id'];?>"><?= $row['nome_setor'];?></option>
				...
				<div class="form-group col-md-4">
				  ...
				  <option value="">Selecione</option>
					<?php foreach($funcoes as  $row){
					  if(!empty($row['id'])){
					  ?>
                    <option <?= ($usuario[0]['funcao']==$row['id']?'selected':'');?> value="<?= $row['id'];?>"><?= $row['descricao'];?></option>
				...
				<div class="form-group col-md-4">
				  ...
				  <input type="email" class="form-control" id="email" value="<?=$usuario[0]['email'];?>" placeholder="Email" name="email">
				</div>
				<div class="form-group col-md-4">
				  <label for="Senha">Senha</label>
				  <input type="password" class="form-control" id="senha" value="<?=$usuario[0]['senha'];?>" placeholder="Senha" name="senha">
				</div>	
			  </div>
